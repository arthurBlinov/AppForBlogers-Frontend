...
export default function UploadProfilePhoto() {
  ...
  //redirect 
  if(profilePhoto) return <Navigate to={`/profile/${id}`}/>
  return (
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form className="space-y-6" onSubmit={formik.handleSubmit}>
            ...
            {appErr || serverErr ? <h2 className="text-center text-red">{serverErr} {appErr}</h2> :
             null}
            <Container>
            <Dropzone
            ...
            <div>
              {loading ? <button
                ...
                rounded-md text-gray-700 bg-gray-500"
              >
                <Upload
